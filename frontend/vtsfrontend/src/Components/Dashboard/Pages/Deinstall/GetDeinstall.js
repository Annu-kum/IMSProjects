...
import { TextField, Table,AlertTitle, TableBody, TableCell, TableContainer,Alert, TableHead, TableRow, Paper, Checkbox, Button } from '@mui/material';
...
const InputTable = () => {
  ...
  const fetchInstallations = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/installation/getinstaller/${millerId}/`, { headers });
            ...
            {installations.length > 0 ? (
              installations.map((installation) => (
                <TableRow key={installation.id}>
                  ...
                  <TableCell>
                    <Controller
                      name={`${installation.id}.DeactivationDate`}
                      control={control}
